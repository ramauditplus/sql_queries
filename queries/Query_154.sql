...
SET perms = (SELECT array_agg(DISTINCT val)
             FROM (SELECT CASE
                              ...
                              when elem = 'get_gstr_3b' then 'get_gstr3b'
                              ...
                              when elem = 'view_sale_quotation' then 'get_sale_quotation'
                              when elem = 'category_mapping' then 'set_category'
                              when elem = 'create_nature_of_payment' then 'create_tds_nature_of_payment'
                              when elem = 'update_nature_of_payment' then 'update_tds_nature_of_payment'
                              when elem = 'delete_nature_of_payment' then 'delete_tds_nature_of_payment'
                              when elem = 'financial_year' then 'create_financial_year'
                              when elem = 'inventory_merge' then 'merge_inventory'
                              when elem = 'sale_incentive' then 'sales_incentive_rangewise'
                              when elem = 'sale_reminder' then 'generate_sale_reminder_order'
                              ...
                              when elem in ('organization_email_config', 'organization_aws_config',
                                            'organization_wanted_note_config')
                                  then 'update_organization_config'
                              ...
                              when elem = 'bank_collection_summary' then array ['bank_collection_status'] || array [elem]
