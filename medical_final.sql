...
-- UUID CHANGES
------------------------------------------------------------------------
select now() as time, 'SETTING UUID AS COLUMN FOR STARTS' as msg;
...
------------------
    ...
    UPDATE inventory b SET unit_uuid = a.id FROM unit a WHERE a.name = 'PCS';
    UPDATE inventory b SET sale_unit_uuid = a.id FROM unit a WHERE a.name = 'PCS';
    UPDATE inventory b SET purchase_unit_uuid = a.id FROM unit a WHERE a.name = 'PCS';
    UPDATE inventory t
...
-------------------------------------------------------------------------------------------------
---- APPLY UUID for batch, voucher, ac_txn, inv_txn
-------------------------------------------------------------------------------------------------
...
$$
    ...
    BEGIN
        LOOP
            UPDATE batch i
            SET section_uuid = a.id
            FROM section a
            WHERE a.old_id = i.section_id
...
------------------
select now() as time, 'UUID_CHANGES FOR INV_TXN MANUFACTURER_ID STARTS' as msg;
...
$$
    ...
    END
$$;
select now() as time, 'UUID_CHANGES FOR INV_TXN MANUFACTURER_ID ENDS' as msg;
------------------
...
------------------
select now() as time, 'UUID_CHANGES FOR INV_TXN SECTION_ID STARTS' as msg;
...
$$
    ...
    END
$$;
select now() as time, 'UUID_CHANGES FOR INV_TXN SECTION_ID ENDS' as msg;
------------------
select now() as time, 'UUID_CHANGES FOR INV_TXN UNIT_ID STARTS' as msg;
